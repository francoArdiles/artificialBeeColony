...
EXE=~/algorithms/abcscp/algorithm/abcscp.py
...
shift 5 || exit 1
CAND_PARAMS=$* #Colony_size, Max_trials, Transfer_function,Binarization_method,alpha#

#We generate a random number using shuf gnu core utils, apparently shuf command is not posix complaint, but dash shell with gnu core utils should work fine. To fully portability in unix systems, this line should be checked, and replaced with a  posix complaint random number generator#
RR=$(shuf -i 0-32000 -n1) 
...
$EXE "${FIXED_PARAMS}" --time 0 --tours 1000 --seed "$SEED" -i "$INSTANCE" "${CAND_PARAMS}" 1> "$STDOUT" 2> "$STDERR"
